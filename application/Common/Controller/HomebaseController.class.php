...
class HomebaseController extends AppframeController {
	...
	protected function check_login(){
	    ...
	    $userInfo = M('users')->where(['id'=>$session_user,'user_type'=>3,'openid'=>$openid])->find();
		if(empty($userInfo)){
            redirect(U('User/Wx/index'));
		}
	}
	...
	protected function  check_user(){
		$session_user=session('user');
	    $user_status=M('users')->where(array("id"=>$session_user))->getField("user_status");

		if($user_status==0){
            $this->display('frozen');die;
		}
	}
